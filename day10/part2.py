...
def find_set_of_next_height(target: str, current_points: List[Coordinate]) -> List[Coordinate]:
    next_points: List[Coordinate] = []
    for point in current_points:
        # left
        if point.x > 0 and rows[point.y][point.x-1] == target:
            next_points.append(Coordinate(row=point.y, column=point.x-1))
        # right
        if point.x < max_x and rows[point.y][point.x+1] == target:
            next_points.append(Coordinate(point.y, point.x+1))
        # up
        if point.y > 0 and rows[point.y-1][point.x] == target:
            next_points.append(Coordinate(point.y-1, point.x))
        # down
...
def check_trails(row, column) -> int:
    heights = "123456789"
    current_points: List[Coordinate] = [Coordinate(row=row, column=column)]
